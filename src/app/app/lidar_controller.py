...
class LidarController(Node):
    ...
    def set_running_srv_callback(self, request, response):
        ...
        :param req: 0关闭，1避障，2跟随，3警卫(0 close, 1 obstacle avoidance, 2 Lidar following, 3 Lidar guarding, 4 move and stop on obstacle)
        :return:
        '''
            ...
            if self.running_mode == 1 and self.timestamp <= time.time():
                ...
                if len(dist_) > 1:
                    ...
                    if dist < 2 and abs(math.degrees(angle)) > 5:  # 控制左右(control the left and right)
                        ...
                        self.pid_yaw.clear()
                    self.mecanum_pub.publish(twist)
            elif self.running_mode == 4:
                # 前进，检测到障碍物就停止，障碍物移开后继续(move forward, stop when an obstacle is detected, resume when it is cleared)
                left_nonzero = left_range.nonzero()
                right_nonzero = right_range.nonzero()
                left_nonan = np.isfinite(left_range[left_nonzero])
                right_nonan = np.isfinite(right_range[right_nonzero])
                ...
                if len(min_dist_left_) > 0 and len(min_dist_right_) > 0:
                    min_dist = min(min_dist_left_.min(), min_dist_right_.min())
