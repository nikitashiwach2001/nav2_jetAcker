...
def launch_setup(context):
    ...
    # Laser scan-matching odometry: measures the robot's real velocity from lidar scans.
    # Feeds the EKF (odom0: odom_rf2o) instead of odom_raw, which only integrates commanded
    # velocity and therefore drifts forward when the robot is physically stuck.
    rf2o_laser_odometry_node = Node(
        ...
        parameters=[{
            ...
            'publish_tf': False,  # odom->base tf is owned by the EKF
            'base_frame_id': 'base_footprint',
            'odom_frame_id': 'odom',
            'init_pose_from_topic': '',
            'freq': 10.0,  # match the LD19 scan rate; higher just logs "Waiting for laser_scans"
        }],
        condition=IfCondition(enable_odom),
    )
