    ...
    def __call__(self, image, result_image):
        ...
        for roi in self.rois:
            ...
            if max_contour_area is not None:
                ...
                box = np.intp(cv2.boxPoints(rect))  # 四个角four corners)
                ...
                cv2.drawContours(result_image, [box], -1, (int(255*roi[-1]), 255, 0), 2)  # 画出四个点组成的矩形(draw a rectangle formed by four points)
        ...
        center_pos = centroid_sum / self.weight_sum  # 按比重计算中心点(calculate the center point based on weight)
