...
# Parallel copy of navigation_base_v4.launch.py that can select the v5 RL controller
# plugin (rl_nav_cpp/RLControllerV5, config/nav2_controller_rl_v5.yaml, model_10496
# checkpoint -- literal copy of v1's controller, only the model differs) via
# use_rl_v5:=true, without touching the v1/v2/v3/v4 files.
...
def launch_setup(context):
    ...
    declare_use_rl_v5_cmd = DeclareLaunchArgument(
        ...
        description='Use the RL v5 (ONNX, model_10496 checkpoint, same config as v1) '
                     'Ackermann controller. Takes priority over use_rl_v4/v3/v2/use_rl/use_teb if true.')

    if use_rl_v5 == 'true':
        controller_param = os.path.join(navigation_package_path, 'config/nav2_controller_rl_v5.yaml')
    elif use_rl_v4 == 'true':
    ...
    load_composable_nodes = LoadComposableNodes(
        ...
        composable_node_descriptions=[
            ...
            ComposableNode(
                ...
                name='bt_navigator',
                parameters=[params_file],
                # goals reach the BT only through goal_point_relay.py (launched by
                # navigation_v5.launch.py), which rewrites a goal heading that would force a
                # minimum-radius loop into the path -- the RViz arrow tool publishes straight
                # to /goal_pose, so without this remap the relay could only serve the
                # "Publish Point" tool. If the relay is not running, publish to /goal_pose_nav.
