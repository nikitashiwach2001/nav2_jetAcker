...
int SpeechProcess::record_params_init(record_handle_t* pcm_handle,record_params_t* params)
{
	...
	/*参数结构体，可用于指定PCM流的配置(a parameter structure can be employed to specify the configuration of a PCM stream)*/
	snd_pcm_hw_params_t *hwparams;

	/*分配硬件参数结构对象，并判断是否分配成功(allocate a hardware parameter structure object and verify if the allocation is successful)*/
	snd_pcm_hw_params_alloca(&hwparams);

	/*对硬件对象进行初始化默认设置(initialize the hardware object with default settings)*/
    if((err = snd_pcm_hw_params_any(pcm_handle->pcm,hwparams)) < 0)
    {
 	...
 	/*
    	设置数据为交叉模式(降噪板默认输出单通道PCM)(set the data to interleaved mode (the noise reduction board's default output is single-channel PCM))
    	INTERLEAVED/NONINTERLEAVED:交叉/非交叉模式。(Interleaved/Non-interleaved mode)
    	表示在多声道数据传输的过程中是采样交叉的模式还是非交叉的模式。(indicating whether the data transfer during multi-channel transmission is in interleaved or non-interleaved mode)
    	对多声道数据，如果采样交叉模式，使用一块buffer即可，其中各声道的数据交叉传输；(For multi-channel data, if using interleaved mode, only one buffer is required, and the data of each channel is transmitted in an interleaved manner)
	如果使用非交叉模式，需要为各声道分别分配一个buffer，各声道数据分别传输。(If using non-interleaved mode, a separate buffer needs to be allocated for each channel, and the data of each channel is transmitted separately)
	*/
    ...
    /*获取格式设置，并设置pcm数据格式(get the format settings and set the PCM data format)*/
    ...
    /*获取声道并设置(obtain channel and set it)*/
    ...
    /*获取采样率并设置(obtain sample rate and set it)*/
    ...
    /*将配置写入驱动程序(write the configuration to the drive program)*/
    ...
    /*准备音频接口(prepare the audio interface)*/
	...
	/*配置一个数据缓冲区用来缓冲数据(configure a data buffer for buffering data)*/
